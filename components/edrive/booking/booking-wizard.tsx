      ...
      {experienceOptions.map((item) => {
        ...
        return (
          <button key={item.id} type="button" onClick={() => onSelect(item.id)} className={cn('group grid overflow-hidden rounded-[1.25rem] border bg-white text-left transition sm:grid-cols-[125px_1fr]', active ? 'border-primary shadow-sm ring-2 ring-primary/10' : 'border-border/80 hover:border-primary/35')} aria-pressed={active}>
            ...
            <div className="flex items-start justify-between gap-3 p-4"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-heading text-lg font-semibold text-foreground">{item.title}</h3>{item.recommended ? <span className="rounded-full bg-accent-200 px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.1em] text-primary-900">Popular</span> : null}</div><p className="mt-1.5 max-w-xl text-xs leading-5 text-muted-foreground">{item.shortDescription}</p><p className="mt-2 text-xs font-semibold text-primary">{item.startingPrice ? `From ${formatAed(item.startingPrice)}` : 'Request pricing'}</p></div><span className={cn('mt-1 flex size-5 shrink-0 items-center justify-center rounded-full border', active ? 'border-primary bg-primary text-white' : 'border-border bg-background')}>{active ? <Check className="size-3" aria-hidden="true" /> : null}</span></div>
          </button>
        );
      })}
    </div>
...
function ContactStep({ draft, onUpdate }: { draft: BookingDraft; onUpdate: (values: Partial<BookingDraft>) => void }) {
  return <div className="grid gap-4"><div className="grid gap-4 sm:grid-cols-2"><label className={fieldLabel}>Full name <span className="relative"><UserRound className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted-foreground" /><Input required value={draft.customerName} onChange={(event) => onUpdate({ customerName: event.target.value })} className="pl-10" placeholder="Your full name" /></span></label><label className={fieldLabel}>Phone or WhatsApp <span className="relative"><Phone className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted-foreground" /><Input required type="tel" value={draft.customerPhone} onChange={(event) => onUpdate({ customerPhone: event.target.value })} className="pl-10" placeholder={companyInfo.whatsappDisplay} /></span></label></div><div className="grid gap-4 sm:grid-cols-2"><label className={fieldLabel}>Email address <span className="font-normal text-muted-foreground">(optional)</span><span className="relative"><Mail className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted-foreground" /><Input type="email" value={draft.customerEmail} onChange={(event) => onUpdate({ customerEmail: event.target.value })} className="pl-10" placeholder="you@example.com" /></span></label><label className={fieldLabel}>Hotel or area <span className="font-normal text-muted-foreground">(optional)</span><span className="relative"><MapPin className="pointer-events-none absolute left-3 top-3.5 size-4 text-muted-foreground" /><Input value={draft.customerHotelOrArea} onChange={(event) => onUpdate({ customerHotelOrArea: event.target.value })} className="pl-10" placeholder="Where are you staying?" /></span></label></div><label className={fieldLabel}>Notes <span className="font-normal text-muted-foreground">(optional)</span><Textarea value={draft.customerNotes} onChange={(event) => onUpdate({ customerNotes: event.target.value })} placeholder="Celebration details, rider experience, or anything else we should know" /></label><p className="rounded-[1.15rem] bg-primary-50 px-4 py-3 text-xs leading-5 text-primary-900">Your details are used only to confirm and manage this booking request.</p></div>;
}
