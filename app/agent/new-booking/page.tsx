...
import { CalendarDays, Check, CheckCircle2, ChevronLeft, ChevronRight, Minus, Package, Plus, RefreshCw, WalletCards } from 'lucide-react';
...
type FormState = { packageId: string; vehicleQuantity: string; guestCount: string; preferredDate: string; preferredTime: string };
const initial: FormState = { packageId: '', vehicleQuantity: '1', guestCount: '1', preferredDate: '', preferredTime: '' };
const steps = [{ title: 'Package', icon: Package }, { title: 'Schedule', icon: CalendarDays }, { title: 'Review', icon: CheckCircle2 }];
...
export default function AgentNewBookingPage() {
  const { profile, walletBalance: balance, refreshingPortal, refreshPortal } = useAgentPortal();
  ...
  const agentContact = profile.contact_person?.trim() || '';
  const agentPhone = profile.phone?.trim() || '';
  const agentEmail = profile.login_email?.trim() || profile.email?.trim() || '';
  const agentIdentityError = !agentContact || !agentPhone || !agentEmail
    ? 'Your B2B profile is missing required contact details. Please contact eDrive support.'
    : '';
  ...
  function next() { const validation = validateStep(); if (validation) { setError(validation); return; } setError(''); setStep((current) => Math.min(current + 1, 2)); }
  async function submit() {
    if (!selected || insufficient || saving) return;
    if (scheduleError) { setError(scheduleError); return; }
    if (agentIdentityError) { setError(agentIdentityError); return; }
    setSaving(true); setError('');
    try {
      const created = await createB2BBooking({ package_id: selected.id, vehicle_quantity: quantity, guest_count: Math.max(Number(form.guestCount) || 1, 1), preferred_date: form.preferredDate, preferred_time: form.preferredTime, customer_name: agentContact, customer_phone: agentPhone, customer_email: agentEmail, customer_hotel_or_area: null, customer_notes: null });
  ...
  if (loading) return <><AgentPageHeader eyebrow="New booking" title="Create a partner booking" description="Select a package and schedule, then review and confirm the booking." /><BookingSkeleton /></>;
...
function PackageStep({ packages, activeCategory, onCategoryChange, selectedId, onSelect, quantity, setQuantity, guestCount, setGuestCount }: {
  ...
  return <section>
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2">
      <QuantityControl label="Vehicle quantity" value={quantity} max={20} onChange={setQuantity} />
      <QuantityControl label="Guest count" value={guestCount} max={100} onChange={setGuestCount} />
    </div>
  </section>;
}
function QuantityControl({ label, value, max, onChange }: { label: string; value: number; max: number; onChange: (value: number) => void }) { return <div><p className="text-sm font-semibold">{label}</p><div className="mt-2 inline-flex h-10 items-center overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm"><button type="button" aria-label={`Decrease ${label.toLowerCase()}`} disabled={value <= 1} onClick={() => onChange(Math.max(1, value - 1))} className="flex h-full w-10 items-center justify-center text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300"><Minus className="size-4" /></button><output aria-live="polite" className="flex h-full min-w-12 items-center justify-center border-x border-slate-200 px-3 text-sm font-bold">{value}</output><button type="button" aria-label={`Increase ${label.toLowerCase()}`} disabled={value >= max} onClick={() => onChange(Math.min(max, value + 1))} className="flex h-full w-10 items-center justify-center text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300"><Plus className="size-4" /></button></div></div>; }
function ReviewStep({ form, selected, quantity, total, scheduleError, identityError, agentContact, agentPhone, agentEmail, partnerLabel }: { form: FormState; selected: PackageRow | null; quantity: number; total: number; scheduleError: string; identityError: string; agentContact: string; agentPhone: string; agentEmail: string; partnerLabel: string }) { const price = selected ? getPackagePricePresentation(selected, 'b2b') : null; return <section><h2 className="font-heading text-lg font-semibold">Review & confirm</h2>{scheduleError ? <div role="alert" className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700">{scheduleError} Return to Schedule to choose an available slot.</div> : null}{identityError ? <div role="alert" className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700">{identityError}</div> : null}{price?.active ? <div className="mt-4"><PackageOfferRibbon pricing={selected!} audience="b2b" /></div> : null}<div className="mt-4 rounded-xl border border-teal-200 bg-teal-50 p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-teal-700">Booked by</p><p className="mt-2 font-semibold text-slate-950">{agentContact || 'Contact details unavailable'}</p><p className="mt-1 text-sm text-slate-600">{agentPhone || '-'}</p><p className="mt-1 text-sm text-slate-600">{agentEmail || '-'}</p><p className="mt-3 border-t border-teal-200 pt-3 text-xs font-semibold text-teal-900">Partner: {partnerLabel}</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><SummaryBox label="Package" value={selected?.title || '-'} />{price?.active ? <SummaryBox label="Offer" value={price.label} /> : null}{price?.active ? <SummaryBox label="Regular B2B unit price" value={formatAed(price.normalPrice)} /> : null}<SummaryBox label={price?.active ? 'Offer B2B unit price' : 'B2B unit price'} value={formatAed(price?.effectivePrice || 0)} /><SummaryBox label="Schedule" value={`${form.preferredDate} | ${form.preferredTime}`} /><SummaryBox label="Vehicles / Guests" value={`${quantity} / ${form.guestCount}`} /><SummaryBox label="Total wallet debit" value={formatAed(total)} /></div></section>; }
...
function BookingSkeleton() { return <div className="mt-4 grid animate-pulse gap-4 lg:grid-cols-[minmax(0,1fr)_310px]"><div><div className="grid grid-cols-3 gap-2">{Array.from({ length: 3 }).map((_, index) => <div key={index} className="h-10 rounded-xl bg-slate-200" />)}</div><div className="mt-4 h-80 rounded-xl bg-white shadow-sm" /></div><div className="h-72 rounded-xl bg-white shadow-sm" /></div>; }
