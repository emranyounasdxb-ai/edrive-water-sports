...
export function AdminB2BFinancePage() {
  ...
  return <section className="w-full space-y-4 px-4 py-4 sm:px-5 lg:px-6">
    <CompactPageHeader eyebrow="B2B Finance" title="B2B Wallets" description="Manage partner balances, ledger activity and refund requests." actions={<Button size="sm" variant="outline" disabled={refreshing} onClick={() => load(true)}><RefreshCw className={`size-4 ${refreshing ? 'animate-spin' : ''}`} />Refresh</Button>} />
    ...
    <Card className="rounded-2xl border-border/80"><CardContent className="grid gap-2.5 p-3.5 lg:grid-cols-[minmax(240px,0.8fr)_minmax(360px,1.2fr)]"><label className="relative"><Search className="absolute left-3 top-3 size-4 text-muted-foreground" /><Input className="h-10 pl-9" placeholder="Search partner accounts" value={agentSearch} onChange={(event) => setAgentSearch(event.target.value)} /></label><select className={`${selectClass} h-10`} aria-label="Selected B2B Agent" value={selectedId} onChange={(event) => setSelectedId(event.target.value)}><option value="">All Agents Overview</option>{agentOptions.map((agent) => <option key={agent.id} value={agent.id}>{agent.company_name} | {agent.agent_code || 'No code'} | {agent.status} | {formatAed(walletMap[agent.id] || 0)}</option>)}</select></CardContent></Card>
    ...
    {canView && !selectedAgent ? <>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <Metric label="Agents" value={String(agents.length)} loading={loading} />
        <Metric label="Active" value={String(agents.filter((agent) => agent.status.toLowerCase() === 'active').length)} loading={loading} />
        <Metric label="Wallet" value={formatAed(combinedWalletBalance)} loading={loading} />
        <Metric label="Debits" value={formatAed(totalBookingDebits)} loading={loading} />
        <Metric label="Pending" value={String(pendingRequestCount)} loading={loading} />
      </div>
      <Card className="overflow-hidden rounded-2xl border-border/80"><CardHeader className="px-4 py-3"><CardTitle className="whitespace-nowrap text-base">Partner Accounts</CardTitle></CardHeader><CardContent className="p-0"><Table className="table-fixed"><TableHeader><TableRow><TableHead className="w-[24%]">Company</TableHead><TableHead className="w-[12%]">Agent Code</TableHead><TableHead className="w-[10%]">Status</TableHead><TableHead className="w-[15%] text-right">Wallet Balance</TableHead><TableHead className="w-[10%] text-center">Pending</TableHead><TableHead className="w-[17%]">Last Activity</TableHead><TableHead className="w-[12%] text-right">Action</TableHead></TableRow></TableHeader><TableBody>{loading ? <TableRow><TableCell colSpan={7} className="py-7 text-center">Loading partner accounts...</TableCell></TableRow> : null}{!loading && !visibleAgents.length ? <TableRow><TableCell colSpan={7} className="py-7 text-center text-muted-foreground">No partner accounts match the current search.</TableCell></TableRow> : visibleAgents.map((agent) => { const pending = refunds.filter((request) => request.b2b_agent_id === agent.id && request.status === 'Pending').length; const lastActivity = ledger.find((entry) => entry.b2b_agent_id === agent.id); return <TableRow key={agent.id}><TableCell><OverflowText value={agent.company_name} maxWidth="max-w-[15rem]" className="font-semibold text-primary" /></TableCell><TableCell className="whitespace-nowrap font-mono text-xs">{agent.agent_code || '-'}</TableCell><TableCell><Badge variant={agent.status.toLowerCase() === 'active' ? 'success' : 'secondary'}>{agent.status}</Badge></TableCell><TableCell className="whitespace-nowrap text-right font-semibold tabular-nums">{formatAed(walletMap[agent.id] || 0)}</TableCell><TableCell className="text-center">{pending ? <Badge variant="warning">{pending}</Badge> : '0'}</TableCell><TableCell className="whitespace-nowrap text-xs">{lastActivity ? formatDate(lastActivity.created_at) : 'No activity'}</TableCell><TableCell className="text-right"><Button size="sm" variant="outline" onClick={() => setSelectedId(agent.id)}>View</Button></TableCell></TableRow>; })}</TableBody></Table></CardContent></Card>
      <Card className="overflow-hidden rounded-2xl border-border/80"><CardHeader className="px-4 py-3"><CardTitle className="whitespace-nowrap text-base">Refund Requests</CardTitle></CardHeader><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Requested</TableHead><TableHead>Booking Ref</TableHead><TableHead>Company</TableHead><TableHead>Agent Code</TableHead><TableHead>Type</TableHead><TableHead>Reason</TableHead><TableHead className="text-right">Original Amount</TableHead><TableHead className="text-right">Refund Amount</TableHead><TableHead>Status</TableHead><TableHead>Notes</TableHead><TableHead>Action</TableHead></TableRow></TableHeader><TableBody>{loading ? <TableRow><TableCell colSpan={11} className="py-7 text-center">Loading requests...</TableCell></TableRow> : null}{!loading && !refunds.length ? <TableRow><TableCell colSpan={11} className="py-7 text-center text-muted-foreground">No refund requests.</TableCell></TableRow> : refunds.map((request) => { const booking = bookingMap[request.booking_request_id]; const agent = agentMap[request.b2b_agent_id]; const pending = request.status === 'Pending'; return <TableRow key={request.id} className={pending ? 'bg-amber-50/70' : undefined}><TableCell className="whitespace-nowrap text-xs">{formatDate(request.requested_at)}</TableCell><TableCell className="whitespace-nowrap font-mono text-xs">{booking?.booking_code || '-'}</TableCell><TableCell><OverflowText value={agent?.company_name} maxWidth="max-w-40" className="font-semibold text-primary" /></TableCell><TableCell className="whitespace-nowrap font-mono text-xs">{agent?.agent_code || '-'}</TableCell><TableCell className="whitespace-nowrap font-semibold">{request.request_type === 'no_show_refund' ? 'No Show Refund' : 'Cancellation'}</TableCell><TableCell><OverflowText value={request.reason} maxWidth="max-w-44" maxCharacters={24} /></TableCell><TableCell className="whitespace-nowrap text-right tabular-nums">{formatAed(booking?.amount_received_aed || booking?.total_amount || 0)}</TableCell><TableCell className="whitespace-nowrap text-right tabular-nums">{formatAed(request.requested_amount_aed)}</TableCell><TableCell><RequestStatus status={request.status} /></TableCell><TableCell><OverflowText value={[request.agent_note, request.decision_note].filter(Boolean).join(' | ')} maxWidth="max-w-40" maxCharacters={22} /></TableCell><TableCell>{canWrite && pending ? <div className="flex gap-1"><Button size="sm" onClick={() => { setDecision({ request, value: 'Approved' }); setNote(''); }}><CheckCircle2 className="size-4" /><span className="sr-only">Approve</span></Button><Button size="sm" variant="outline" onClick={() => { setDecision({ request, value: 'Rejected' }); setNote(''); }}><XCircle className="size-4" /><span className="sr-only">Reject</span></Button></div> : <span className="whitespace-nowrap text-xs text-muted-foreground">View only</span>}</TableCell></TableRow>; })}</TableBody></Table></CardContent></Card>
    </> : null}
    {canView && selectedAgent ? <>
...
function Metric({ label, value, loading }: { label: string; value: string; loading: boolean }) { return <CompactKpiCard label={label} value={loading ? '—' : value} icon={WalletCards} />; }
