        ...
        return (
          <div key={entry.id} className="grid gap-2.5 px-4 py-3 sm:grid-cols-[auto_1fr_auto] sm:items-center">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><p className="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-800">{walletTransactionLabel(entry.transaction_type, entry.description)}</p>{entry.booking_request_id && bookingCodes[entry.booking_request_id] ? <span className="rounded bg-slate-100 px-2 py-0.5 font-mono text-[11px] text-slate-600">{bookingCodes[entry.booking_request_id]}</span> : null}</div>
              <p className="mt-1 truncate text-xs text-slate-500" title={entry.description}>{entry.description}</p>
