...
declare
  ...
  v_master_changed boolean := tg_op = 'INSERT';
begin
  if tg_op = 'UPDATE' then
    v_master_changed := new.vehicle_code is distinct from old.vehicle_code
      or new.reg_no is distinct from old.reg_no
      or new.device_imei is distinct from old.device_imei
      or new.chassis_vin is distinct from old.chassis_vin
  ...
  -- Existing legacy rows with blank registration remain readable, but any master-data save
  -- must supply the real registration number.
  if v_master_changed and length(v_reg) < 3 then
    raise exception 'Registration number is required for every fleet unit.';
  end if;
  ...
  if lower(coalesce(new.status::text, 'available')) not in (
  ...
  if lower(coalesce(new.status::text, '')) = 'available' and new.registration_expiry is not null and new.registration_expiry < current_date then
    raise exception 'A fleet unit with expired registration cannot be marked Available.';
  end if;

  if lower(coalesce(new.status::text, '')) = 'available' and new.insurance_expiry is not null and new.insurance_expiry < current_date then
    raise exception 'A fleet unit with expired insurance cannot be marked Available.';
  end if;

  new.vehicle_code := v_code;
  if v_reg <> '' then new.reg_no := v_reg; end if;
...
begin
  ...
  if length(v_reg) < 3 then raise exception 'Registration number is required for every fleet unit.'; end if;
  ...
  if p_vehicle_id is null then
    insert into public.vehicles (
      ...
      v_code, v_name, v_type, v_name, lower(v_code), v_type,
      left(coalesce(p_payload->>'description', ''), 1000),
      0, v_location, v_capacity, v_status,
      left(coalesce(p_payload->>'brand', ''), 80), left(coalesce(p_payload->>'model', ''), 80), v_year,
      v_reg, v_registration_expiry, nullif(left(btrim(coalesce(p_payload->>'insurance_number', '')), 80), ''), v_insurance_expiry, v_imei,
    ...
    if v_id is null then raise exception 'Fleet unit was not found.'; end if;
  end if;
