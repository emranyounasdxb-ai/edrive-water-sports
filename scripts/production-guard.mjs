...
const myBookingPage = read('app/(public)/my-booking/page.tsx');
...
const packageOfferMigration = read('supabase/05-package-offers.sql');
const publicOfferRpc = packageOfferMigration.split('create function public.get_public_packages')[1]?.split('revoke all on function public.get_public_packages')[0] || '';
const b2bOfferRpc = packageOfferMigration.split('create or replace function public.get_b2b_agent_packages')[1]?.split('revoke all on function public.get_b2b_agent_packages')[0] || '';
...
assert(['offer_enabled', 'offer_name', 'b2c_offer_price', 'b2b_offer_price'].every((field) => packageOfferMigration.includes(field)), 'Package offer migration must add all four additive offer fields.');
assert(packageOfferMigration.includes('v_package.b2c_offer_price') && packageOfferMigration.includes('v_package.b2b_offer_price'), 'Server booking RPCs must use audience-specific offer prices.');
assert(/returns table \([\s\S]*?offer_enabled boolean, offer_name text, b2c_offer_price numeric\s*\)/.test(packageOfferMigration), 'The public package RPC must return only B2C offer fields.');
assert(packageOfferMigration.includes('get_b2b_agent_packages()') && packageOfferMigration.includes("grant execute on function public.get_b2b_agent_packages() to authenticated"), 'B2B package pricing must use the authenticated Agent-only RPC.');
assert(!packageOfferMigration.includes('grant execute on function public.get_b2b_agent_packages() to anon'), 'Anonymous users must never execute the B2B package-pricing RPC.');
assert(publicOfferRpc.includes('offer_state.is_valid') && publicOfferRpc.includes('case when offer_state.is_valid then btrim(p.offer_name) else null end') && publicOfferRpc.includes('case when offer_state.is_valid then p.b2c_offer_price::numeric else null end'), 'The public package RPC must suppress disabled or invalid offer labels and prices.');
assert(!publicOfferRpc.includes('p.b2b_price::numeric') && !/then p\.b2b_offer_price::numeric/.test(publicOfferRpc), 'The public package RPC must never return B2B pricing.');
assert(b2bOfferRpc.includes('offer_state.is_valid') && b2bOfferRpc.includes('case when offer_state.is_valid then btrim(p.offer_name) else null end') && b2bOfferRpc.includes('case when offer_state.is_valid then p.b2b_offer_price::numeric else null end'), 'The B2B package RPC must suppress disabled or invalid offer labels and prices.');
assert(packagesPage.includes("offer_name: values.offerName.trim() || null") && packagesPage.includes('b2b_offer_price: values.b2bOfferPrice'), 'Package Management must retain saved offer values while an offer is disabled.');
assert(agentNewBooking.includes("rpc('get_b2b_agent_packages')") && !agentNewBooking.includes("from('packages')"), 'The Agent booking page must load confidential prices through the secured B2B RPC.');
assert(packageOfferMigration.includes("conrelid = 'public.packages'::regclass"), 'Package offer constraint checks must be scoped to public.packages.');
...
assert(metaComponent.includes("const pixelId = '1139005827091005'"), 'Meta Pixel must retain the approved public ID.');
assert(metaComponent.includes("script.src = 'https://connect.facebook.net/en_US/fbevents.js'") && metaComponent.includes('script.async = true'), 'Meta must use one asynchronous top-level loader.');
assert(metaComponent.includes("fbq('set', 'autoConfig', false, pixelId)") && metaComponent.includes("fbq('init', pixelId)"), 'Meta must initialize the approved Pixel once without automatic matching.');
assert(!metaComponent.includes('iframe') && !metaComponent.includes('postMessage') && !fs.existsSync(path.join(root, 'public/meta-pixel-frame.html')), 'The retired iframe Pixel architecture must not return.');
assert((metaComponent.match(/connect\.facebook\.net\/en_US\/fbevents\.js/g) || []).length === 1, 'Only one Meta loader may exist.');
assert(!layout.includes('<noscript'), 'Do not add an unconditional tracking fallback that bypasses sensitive URL checks.');
assert((bookingSuccess.match(/trackBookingLead\(request.bookingCode\)/g) || []).length === 2, 'Booking Lead must exist in both successful save branches only.');
assert(/setSaveStatus\('saved'\);\s*setSaveMessage\(messages.requestReceived\);\s*trackBookingLead\(request.bookingCode\)/.test(bookingSuccess), 'Booking Lead must follow the successful save state.');
assert((contactForm.match(/trackMetaEvent\('Lead'\)/g) || []).length === 2, 'Lead must follow both successful contact-form persistence paths only.');
assert(metaComponent.includes("trackMetaEvent('Contact')") && metaComponent.includes("href.startsWith('tel:')") && metaComponent.includes('whatsapp\\.com'), 'Contact must remain limited to existing direct-contact links.');
assert(metaComponent.includes('trackBookingInteraction()') && bookingWizard.includes('trackBookingInteraction()'), 'First booking engagement must emit a deduplicated Contact from a booking CTA or direct package selection.');
assert(metaComponent.includes("event === 'BookingDateSelected' ? 'trackCustom' : 'track'"), 'BookingDateSelected must use Meta custom-event dispatch.');
assert(bookingWizard.includes("trackMetaCustomEvent('BookingDateSelected')") && bookingWizard.includes('onDateSelected();'), 'BookingDateSelected must follow a genuine date selection only.');
assert(bookingWizard.includes("step === 3 && canContinue") && bookingWizard.includes("trackMetaEvent('InitiateCheckout')"), 'InitiateCheckout must follow valid Date and Time progression only.');
assert(bookingWizard.includes("step === 4 && canContinue") && bookingWizard.includes("trackMetaEvent('CompleteRegistration')"), 'CompleteRegistration must follow valid Contact Details progression only.');
assert(!metaComponent.includes('trackMetaInitiateCheckout') && !metaHelper.includes('trackMetaInitiateCheckout') && !metaHelper.includes("'Schedule'"), 'Automatic checkout and the replaced Schedule event must not return.');
assert(!metaHelper.includes("'Purchase'") && !metaComponent.includes("'Purchase'"), 'Purchase must not fire without a proven paid public-order signal.');
...
for (const prefix of ['', '/ar', '/ru']) {
  ...
  for (const route of ['/booking/?package=jet-ski&category=jet_ski&capacity=2&duration=30', '/booking/?fbclid=CAMPAIGN', '/booking/?category=jetski&utm_campaign=summer', '/?utm_source=facebook', '/rentals/?fbclid=CAMPAIGN']) {
...
const bookingMetaUrl = new URL(metaScope.window.location.href);
metaScope.exports.trackMetaPageView(bookingMetaUrl);
metaScope.exports.trackMetaPageView(bookingMetaUrl);
metaScope.exports.trackBookingInteraction();
metaScope.exports.trackBookingInteraction();
metaScope.exports.trackMetaCustomEvent('BookingDateSelected');
metaScope.exports.trackMetaEvent('InitiateCheckout');
metaScope.exports.trackMetaEvent('CompleteRegistration');
metaScope.exports.trackBookingLead('LOCAL_ONLY');
metaScope.exports.trackBookingLead('LOCAL_ONLY');
assert(JSON.stringify(observedMetaEvents) === '[["PageView"],["Contact"],["BookingDateSelected"],["InitiateCheckout"],["CompleteRegistration"],["Lead"]]', 'Booking funnel must preserve the requested event order and deduplicate booking interaction and Lead.');
metaScope.window.location = { href: 'https://edrivedubai.ae/rentals/?utm_source=facebook', pathname: '/rentals/' };
const contentMetaUrl = new URL(metaScope.window.location.href);
metaScope.exports.trackMetaContentView(contentMetaUrl);
metaScope.exports.trackMetaContentView(contentMetaUrl);
assert(JSON.stringify(observedMetaEvents.slice(-1)) === '[["ViewContent"]]', 'Meaningful service views must emit one ViewContent event.');
metaScope.window.location = { href: 'https://edrivedubai.ae/my-booking/?ref=TEST123', pathname: '/my-booking/' };
metaScope.exports.trackBookingLead('ANOTHER_LOCAL_KEY');
assert(observedMetaEvents.length === 7, 'Protected pages must not emit booking Lead events.');
