...
function RideCard({ booking, vehicles, onSaved }: { booking: BookingRow; vehicles: VehicleOption[]; onSaved: () => Promise<void> }) {
  ...
  const status = cardStatus(booking); const inProgress = status === 'In Progress'; const completed = status === 'Completed'; const noShow = status === 'No Show'; const actionAllowed = canOperateBooking(booking); const assignedVehicle = text(booking.assigned_vehicle_name, '');
  const vehicleOptions = useMemo(() => {
    const names = new Set<string>();
    vehicles.filter(isVehicleAvailable).forEach((item) => names.add(vehicleValue(item)));
    if (assignedVehicle) names.add(assignedVehicle);
    return Array.from(names).filter(Boolean).sort();
  }, [assignedVehicle, vehicles]);
  const selectedAvailable = !vehicle || Boolean(findVehicle(vehicles, vehicle) && isVehicleAvailable(findVehicle(vehicles, vehicle)!)) || vehicle === assignedVehicle;
  async function startRide() {
    if (!vehicle.trim()) { setError('Please select available vehicle first.'); return; }
    const selected = findVehicle(vehicles, vehicle);
    if (!selected || !isVehicleAvailable(selected)) { setError('This vehicle is not available. Please select another available vehicle.'); return; }
    setSaving(true); setError('');
    try {
  ...
  return <div className="rounded-[1.15rem] border border-border bg-white p-3 shadow-[0_10px_24px_rgba(8,37,50,0.05)] sm:p-4"><button type="button" onClick={() => setExpanded((value) => !value)} className="flex w-full items-start justify-between gap-3 text-left"><div className="min-w-0"><p className="text-xs font-bold text-primary">{dateLabel(booking.preferred_date)} · {text(booking.preferred_time, 'Time pending')}</p><h3 className="mt-1 break-words font-heading text-base font-semibold leading-tight text-foreground">{packageLabel(booking)}</h3><p className="mt-1 text-sm font-semibold text-muted-foreground">{text(booking.customer_name, 'Guest')}</p></div><span className={`shrink-0 rounded-full border px-2.5 py-1 text-[11px] font-bold ${statusTone(status)}`}>{status}</span></button><div className="mt-3 grid grid-cols-2 gap-2"><Detail label="Vehicle" value={vehicle || 'Not selected'} sub={inProgress || completed ? 'Assigned vehicle' : 'Select before ride'} /><Detail label="Payment" value={formatAed(totalAmount(booking))} sub={paymentSubText(booking)} /></div>{expanded ? <div className="mt-3 grid grid-cols-2 gap-2"><Detail label="Booking" value={bookingCode(booking)} /><Detail label="Ride" value={rideDetails(booking)} sub={text(booking.customer_phone || booking.customer_email, '')} /></div> : null}<div className="mt-3 flex flex-wrap gap-2"><Badge variant="secondary">{isB2BBooking(booking) ? `B2B · ${text(booking.b2b_agent_name, 'Agent')}` : 'Direct Sale'}</Badge><button type="button" onClick={() => setExpanded((value) => !value)} className="rounded-full border border-border bg-white px-3 py-1 text-xs font-bold text-muted-foreground">{expanded ? 'Less' : 'Details'} {expanded ? <ChevronUp className="inline size-3" /> : <ChevronDown className="inline size-3" />}</button></div>{error ? <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{error}</p> : null}<div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end"><label className="grid gap-1.5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Vehicle<select value={vehicle} onChange={(event) => setVehicle(event.target.value)} disabled={!actionAllowed || inProgress} className={selectClass}><option value="">Select available vehicle</option>{vehicleOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select>{actionAllowed && !inProgress && vehicle && !selectedAvailable ? <span className="text-[11px] font-semibold normal-case tracking-normal text-red-700">Selected vehicle is not available.</span> : null}</label><div className="flex flex-col gap-2 sm:flex-row">{actionAllowed && !inProgress ? <Button type="button" onClick={startRide} disabled={saving || !vehicle || !selectedAvailable} className="rounded-full"><Save className="size-4" />{saving ? 'Starting...' : 'Start Ride'}</Button> : null}{inProgress ? <Button type="button" onClick={() => setCompletionOpen(true)} className="rounded-full bg-emerald-600 hover:bg-emerald-700"><CheckCircle2 className="size-4" />Complete Ride</Button> : null}{actionAllowed ? <Button type="button" variant="outline" onClick={markNoShow} disabled={saving} className="rounded-full border-red-200 bg-red-50 text-red-700 hover:bg-red-100"><AlertCircle className="size-4" />No Show</Button> : null}{!actionAllowed && isFutureBooking(booking) ? <Button type="button" disabled variant="outline" className="rounded-full bg-white">Upcoming</Button> : null}{completed ? <Button type="button" disabled className="rounded-full bg-emerald-600"><CheckCircle2 className="size-4" />Completed</Button> : null}{noShow ? <Button type="button" disabled variant="outline" className="rounded-full border-red-200 bg-red-50 text-red-700"><AlertCircle className="size-4" />No Show</Button> : null}</div></div>{completionOpen ? <PaymentModal booking={booking} onClose={() => setCompletionOpen(false)} onComplete={completeRide} /> : null}</div>;
}

function ManagerAssignedRidesPage({ manager }: { manager: ManagerProfile }) {
  const [bookings, setBookings] = useState<BookingRow[]>([]); const [vehicles, setVehicles] = useState<VehicleOption[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [query, setQuery] = useState(''); const [filter, setFilter] = useState<RideFilter>('today');
  async function loadData() {
    setLoading(true); setError('');
    try {
      const [bookingResult, vehicleResult] = await Promise.all([supabase.from(bookingRequestsTable).select('*').order('preferred_date', { ascending: true }).limit(500), supabase.from('vehicles').select('vehicle_code,vehicle_name,vehicle_type,status').order('vehicle_code', { ascending: true }).limit(500)]);
    ...
    } catch (loadError) { setBookings([]); setError(loadError instanceof Error ? loadError.message : 'Unable to load assigned rides.'); } finally { setLoading(false); }
  }
