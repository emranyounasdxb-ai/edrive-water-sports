...
import { DashboardActionList, DashboardPanel, DashboardProgressList } from './shared/dashboard-visuals';
...
export function AdminMaintenancePage() {
  ...
  const attention = vehicles.filter((vehicle) => isMaintenance(vehicle) || Boolean(expiryLabel(vehicle))).slice(0, 6);

  return <section className="w-full space-y-3 overflow-hidden px-4 py-4 sm:px-6 lg:px-8 xl:px-10"><CompactPageHeader eyebrow="Fleet Operations" title="Maintenance Command Center" description="Fleet availability, lifecycle status and vehicles requiring attention." actions={<Button type="button" size="sm" variant="outline" onClick={refresh}><RefreshCw className="size-4" />Refresh</Button>} />{error ? <p className="rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</p> : null}<div className="grid grid-cols-2 gap-2 xl:grid-cols-5"><MetricCard title="Available" value={String(available)} icon={CalendarDays} /><MetricCard title="Maintenance" value={String(maintenance)} icon={Wrench} /><MetricCard title="Booked" value={String(booked)} icon={Ship} /><MetricCard title="Expiry Alerts" value={String(expirySoon)} icon={Wrench} /><MetricCard title="For Sale" value={String(forSale)} icon={Ship} /></div><div className="grid gap-3 lg:grid-cols-2"><DashboardPanel title="Fleet Health" description="Current lifecycle status distribution"><DashboardProgressList items={[{ label: 'Available', value: available, color: '#16a34a' }, { label: 'Booked', value: booked, color: '#0891b2' }, { label: 'Maintenance', value: maintenance, color: '#ef4444' }, { label: 'For Sale', value: forSale, color: '#94a3b8' }]} empty="No fleet records are available." /></DashboardPanel><DashboardPanel title="Needs Attention" description="Maintenance and registration expiry priorities"><DashboardActionList items={attention.map((vehicle) => ({ title: `${expiryLabel(vehicle) || 'Maintenance'} · ${registration(vehicle)}`, meta: `${vehicleName(vehicle)} · ${vehicleType(vehicle)}`, value: 'Review', href: '/admin/maintenance', tone: expiryLabel(vehicle) === 'Expired' ? 'critical' : 'warning' }))} /></DashboardPanel></div><Card className="overflow-hidden rounded-2xl border-border/80 bg-white"><CardHeader className="gap-3 border-b border-border/70 bg-[#F7FAFA] px-4 py-3 lg:flex-row lg:items-center lg:justify-between"><div><CardTitle className="font-heading text-base font-semibold">Fleet control list</CardTitle><p className="text-[11px] font-semibold text-muted-foreground">{loading ? 'Loading vehicles...' : `Showing ${filtered.length} of ${vehicles.length}`}</p></div><div className="relative w-full lg:max-w-xs"><Search className="pointer-events-none absolute left-3 top-2.5 size-4 text-muted-foreground" aria-hidden="true" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search code, type, reg, tracker..." className="h-9 rounded-full bg-white pl-9 text-xs font-semibold" /></div></CardHeader><div className="flex gap-1.5 overflow-x-auto border-b border-border/70 bg-white px-3 py-2">{filters.map((item) => <button key={item.id} type="button" onClick={() => setFilter(item.id)} className={`h-7 whitespace-nowrap rounded-full border px-2.5 text-[11px] font-bold transition ${filter === item.id ? 'border-primary bg-primary text-white' : 'border-border bg-white text-muted-foreground hover:border-primary/40 hover:text-primary-900'}`}>{item.label} <span className={filter === item.id ? 'text-white/75' : 'text-muted-foreground'}>{item.count}</span></button>)}</div><CardContent className="p-0">{loading ? <div className="px-4 py-6 text-center text-xs font-semibold text-muted-foreground">Loading fleet...</div> : null}{!loading && filtered.length === 0 ? <div className="px-4 py-6 text-center text-xs font-semibold text-muted-foreground">No vehicles match the selected filters.</div> : null}{!loading && filtered.length > 0 ? <><div className="hidden border-b border-border/70 bg-[#F7FAFA] px-3 py-2 text-[9px] font-bold uppercase tracking-[0.16em] text-muted-foreground lg:grid lg:grid-cols-[0.9fr_1.2fr_1fr_0.9fr_1fr_1fr_1.1fr]"><span>Vehicle</span><span>Type</span><span>Reg</span><span>Tracker</span><span>Expiry</span><span>Status</span><span className="text-right">Action</span></div><div>{filtered.map((vehicle) => <VehicleRowItem key={asText(vehicle.id || vehicle.vehicle_code || vehicle.vehicle_name)} vehicle={vehicle} onView={() => setSelected(vehicle)} onStatus={updateStatus} />)}</div></> : null}</CardContent></Card>{selected ? <VehicleProfileModal vehicle={selected} onClose={() => setSelected(null)} onStatus={updateStatus} /> : null}</section>;
}
