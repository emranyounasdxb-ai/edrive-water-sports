  ...
  const nav = (
    <nav aria-label="Agent portal navigation" className="space-y-1">
      {navigation.map(({ label, href, icon: Icon, exact }) => {
        ...
        return <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={cn('relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-400', active ? 'bg-teal-50 text-teal-900 before:absolute before:inset-y-2 before:left-0 before:w-1 before:rounded-full before:bg-teal-600' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950')}><Icon className={cn('size-4', active && 'text-teal-700')} aria-hidden="true" />{label}</Link>;
      })}
    </nav>
  );
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(20,184,166,0.10),transparent_30%),#f4f7f8] text-slate-950">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-slate-200 bg-white/95 p-4 backdrop-blur-xl lg:flex lg:flex-col">
        ...
        <div className="space-y-1 border-t border-slate-200 pt-4">
          <a href="tel:+97146113114" className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-teal-50 hover:text-teal-900"><Headphones className="size-4 text-teal-700" />Partner Support</a>
      ...
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur-xl lg:hidden">
        <Link href="/agent"><BrandMark compact /></Link>
        <div className="ml-auto min-w-0 text-right"><p className="text-[9px] font-bold uppercase tracking-wider text-teal-700">Balance</p><p className="truncate font-heading text-sm font-semibold">{walletBalance === undefined ? '-' : formatAed(walletBalance || 0)}</p></div>
        <Button type="button" variant="outline" size="icon" className="shrink-0" aria-label="Open portal menu" onClick={() => setMobileOpen(true)}><Menu className="size-5" /></Button>
      </header>
      {mobileOpen ? <div className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-sm lg:hidden" onMouseDown={() => setMobileOpen(false)}><aside className="h-full w-[86%] max-w-sm overflow-y-auto bg-white p-4 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><BrandMark compact /><Button type="button" variant="ghost" size="icon" aria-label="Close portal menu" onClick={() => setMobileOpen(false)}><X /></Button></div><div className="my-5 rounded-2xl border border-teal-100 bg-teal-50/60 p-4"><p className="truncate font-bold">{profile.company_name}</p><div className="mt-2 flex items-center gap-2"><span className="font-mono text-xs text-slate-600">{profile.agent_code || 'B2B Partner'}</span><AgentStatusBadge status={profile.status} /></div><p className="mt-3 border-t border-teal-100 pt-3 text-xs font-semibold text-slate-500">Available balance</p><p className="font-heading text-xl font-semibold">{walletBalance === undefined ? '-' : formatAed(walletBalance || 0)}</p></div>{nav}<div className="mt-6 border-t pt-4"><a href="tel:+97146113114" className="flex items-center gap-2 py-2 text-sm font-semibold text-teal-800"><Headphones className="size-4" />+971 4 611 3114</a><Link href="/contact" onClick={() => setMobileOpen(false)} className="flex items-center gap-2 py-2 text-sm font-semibold text-slate-700"><CircleHelp className="size-4" />Contact eDrive</Link><button type="button" onClick={logout} className="mt-2 flex items-center gap-2 py-2 text-sm font-semibold text-red-700"><LogOut className="size-4" />Logout</button></div></aside></div> : null}
