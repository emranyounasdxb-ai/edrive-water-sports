...
export function AdminVehiclesPage() { const { items, add } = useLocalRecords(storageKeys.vehicles); return <ManagementPage label="Vehicles / Fleet" title="Vehicle master list" text="Add real Jet Skis and Jet Cars with registration, tracker, image, capacity, status, and maintenance notes." action="Add Vehicle" metrics={[["Jet Skis", String(items.filter((i) => i.type === 'Jet Ski').length), Ship], ["Jet Cars", String(items.filter((i) => i.type === 'Jet Car').length), Ship], ["Maintenance", String(items.filter((i) => i.status === 'Maintenance').length), Settings]]} fields={vehicleFields} items={items} columns={['code', 'regNo', 'type', 'deviceImei', 'expiryDate', 'status']} headers={['Vehicle Code', 'REG. No.', 'Type', 'DEVICE IMEI', 'Expiry Date', 'Status']} empty="No vehicles added yet." onAdd={add} />; }
