...
function scheduleSortValue(booking: BookingRow) {
  return `${dateKey(booking.preferred_date) || '9999-12-31'} ${text(booking.preferred_time, '23:59')}`;
}
...
function canOperateBooking(booking: BookingRow) {
  ...
  return status !== 'Completed' && status !== 'No Show' && !isFutureBooking(booking);
}

function hasRideTimePassed(booking: BookingRow, now = new Date()) {
  const rideDate = dateKey(booking.preferred_date);
...
function canMarkNoShow(booking: BookingRow) {
  return cardStatus(booking) === 'Confirmed' && canOperateBooking(booking) && hasRideTimePassed(booking);
}
...
function expectedRideType(booking: BookingRow): RideType {
  ...
  return (sources.map(normalizeRideType).find(Boolean) || '') as RideType;
}
...
function paymentSubText(booking: BookingRow) {
  ...
  if (status === 'Completed') {
    if (isB2BBooking(booking)) return `B2B pending ${formatAed(pendingAmount(booking))}`;
    if (isCompanyReceived(booking)) return 'Settled with company';
    return `Collected ${formatAed(receivedAmount(booking))}`;
  }
...
function PaymentModal({ booking, onClose, onComplete }: { booking: BookingRow; onClose: () => void; onComplete: (values: CompletionValues) => Promise<void> }) {
  ...
  async function submit() {
    ...
    setError('');
    try {
      const lockedMethod: CompletionValues['method'] = isB2B ? 'B2B Invoice' : method;
      if (!isB2B && lockedMethod === 'B2B Invoice') throw new Error('Direct bookings can only use Cash or Card.');
      const numeric = isB2B ? 0 : Number(amount || 0);
      ...
      <div className="w-full max-w-md rounded-[1.5rem] bg-white p-4 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 font-heading text-xl font-semibold text-foreground">Payment details</h3>
            <p className="mt-1 text-sm text-muted-foreground">{formatAed(total)} total</p>
          </div>
        ...
        {isB2B ? <div className="mt-4 rounded-2xl border border-primary/20 bg-primary-50 p-3"><p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">B2B Invoice</p><p className="mt-1 text-sm font-semibold text-foreground">No guest payment is required. Complete the ride and send {formatAed(total)} to the B2B agent ledger.</p></div> : null}
        {error ? <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{error}</p> : null}
        <div className="mt-4 grid gap-3">
          {isB2B ? (
            <Detail label="Payment Method" value="B2B Invoice" sub="Locked for this B2B booking" />
          ) : (
            <label className="grid gap-1.5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Method
              <select value={method} onChange={(event) => setMethod(event.target.value as CompletionValues['method'])} className={selectClass}>
                <option value="Cash">Cash</option>
                <option value="Card">Card</option>
              </select>
            </label>
          )}
          <label className="grid gap-1.5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Amount
            <Input value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" disabled={isB2B} className="h-10 rounded-xl bg-white" />
          </label>
          {!isB2B && method === 'Card' ? <label className="grid gap-1.5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Reference<Input value={reference} onChange={(event) => setReference(event.target.value)} className="h-10 rounded-xl bg-white" /></label> : null}
          <label className="grid gap-1.5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Note<Input value={note} onChange={(event) => setNote(event.target.value)} className="h-10 rounded-xl bg-white" /></label>
          <Button type="button" onClick={submit} disabled={saving} className="rounded-full">{saving ? 'Saving...' : isB2B ? 'Complete Ride & Create B2B Invoice' : 'Complete Ride'}</Button>
        </div>
      </div>
    </div>
  );
...
function NoShowModal({ booking, saving, onClose, onConfirm }: { booking: BookingRow; saving: boolean; onClose: () => void; onConfirm: (reason: string, note: string) => Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-3 sm:items-center">
      <div className="w-full max-w-md rounded-[1.5rem] bg-white p-4 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-red-700">No Show</p><h3 className="mt-1 font-heading text-xl font-semibold text-foreground">Mark booking as no show?</h3><p className="mt-1 text-sm text-muted-foreground">{bookingCode(booking)} · {text(booking.customer_name, 'Guest')}</p></div>
          <button type="button" onClick={onClose} className="rounded-full border border-border p-2 text-muted-foreground" aria-label="Close"><X className="size-4" /></button>
        </div>
    ...
    <div className="rounded-[1.15rem] border border-border bg-white p-3 shadow-[0_10px_24px_rgba(8,37,50,0.05)] sm:p-4">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          {!actionAllowed && isFutureBooking(booking) ? <Button type="button" disabled variant="outline" className="rounded-full bg-white">Upcoming</Button> : null}
          {completed ? <Button type="button" disabled className="rounded-full bg-emerald-600"><CheckCircle2 className="size-4" />Completed</Button> : null}
          {noShow ? <Button type="button" disabled variant="outline" className="rounded-full border-red-200 bg-red-50 text-red-700"><AlertCircle className="size-4" />No Show</Button> : null}
        </div>
      </div>
...
function ManagerAssignedRidesPage({ manager }: { manager: ManagerProfile }) {
  ...
  const visible = useMemo(() => {
    ...
    return scopedBookings.filter((booking) => matchesFilter(booking, filter)).filter((booking) => !term || [bookingCode(booking), booking.customer_name, booking.customer_phone, packageLabel(booking), booking.assigned_vehicle_name, booking.manager_status, booking.payment_method].some((value) => String(value || '').toLowerCase().includes(term))).sort((a, b) => scheduleSortValue(a).localeCompare(scheduleSortValue(b)));
  ...
  const metrics = useMemo(() => {
    ...
    const outstanding = scopedBookings.filter((booking) => cardStatus(booking) === 'Completed' && !isCompanyReceived(booking));
    const cash = outstanding.filter((booking) => text(booking.payment_method, '').toLowerCase() === 'cash').reduce((sum, booking) => sum + receivedAmount(booking), 0);
    const card = outstanding.filter((booking) => text(booking.payment_method, '').toLowerCase() === 'card').reduce((sum, booking) => sum + receivedAmount(booking), 0);
