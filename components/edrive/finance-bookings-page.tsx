...
const columns: ExportColumn<OperationsBooking>[] = [
  ...
  { heading: 'Vehicle Type', value: (row) => String(row.selected_package_category || row.service_type || '-') }, { heading: 'Booking Source', value: (row) => String(row.booking_source || row.source || '-') },
  ...
  { heading: 'Collection Status', value: (row) => String(row.collection_status || '-') }, { heading: 'Refund AED', value: (row) => reportAmount(row.total_refunded_aed) }
];
...
export function FinanceBookingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to load financial bookings.'); }
  ...
  return <section className="space-y-5">
    <header><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Finance Portal</p><h1 className="mt-1 font-heading text-3xl font-semibold text-primary-900">Financial Bookings</h1><p className="mt-1 text-sm text-muted-foreground">Read-only financial view of public.booking_requests.</p></header>
    <div className="grid gap-3 rounded-3xl bg-white p-4 shadow-sm md:grid-cols-3 xl:grid-cols-5">
      ...
      <Select label="B2B Agent" value={draft.agent_id || ''} onChange={(v) => field('agent_id', v)} options={optionData.agents} />
      <Select label="Package" value={draft.package || ''} onChange={(v) => field('package', v)} values={optionData.packages} />
      <FilterInput label="Vehicle Type" value={draft.vehicle_type || ''} onChange={(v) => field('vehicle_type', v)} />
      <FilterInput label="Payment Method" value={draft.payment_method || ''} onChange={(v) => field('payment_method', v)} />
      ...
      <FilterInput label="Collection Status" value={draft.collection_status || ''} onChange={(v) => field('collection_status', v)} />
    ...
    <div className="overflow-x-auto rounded-3xl bg-white shadow-sm"><table className="w-full min-w-[1350px] text-left text-xs"><thead className="sticky top-0 bg-primary-50 text-primary-900"><tr>{columns.map((column) => <th key={column.heading} className="px-3 py-3">{column.heading}</th>)}</tr></thead><tbody>{loading ? <tr><td colSpan={columns.length} className="p-8 text-center">Loading financial bookings...</td></tr> : rows.length ? rows.map((row) => <tr key={String(row.id)} className="border-t border-border/60">{columns.map((column) => <td key={column.heading} className="whitespace-nowrap px-3 py-3">{column.heading.includes('AED') || ['Total AED', 'Received AED', 'Outstanding AED', 'Refund AED'].includes(column.heading) ? money(Number(column.value(row) || 0)) : String(column.value(row) ?? '-')}</td>)}</tr>) : <tr><td colSpan={columns.length} className="p-8 text-center text-muted-foreground">No financial bookings match the applied filters.</td></tr>}</tbody></table></div>
  </section>;
}
