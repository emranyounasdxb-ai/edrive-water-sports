...
export default function AgentNewBookingPage() {
  ...
  if (success) return <AgentPortalShell profile={profile} walletBalance={balance}><div className="mx-auto max-w-xl py-12"><Card className="rounded-3xl border-emerald-200 shadow-xl"><CardContent className="p-8 text-center"><span className="mx-auto flex size-16 items-center justify-center rounded-full bg-emerald-50 text-emerald-700"><CheckCircle2 className="size-8" /></span><p className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-emerald-700">Booking confirmed</p><h1 className="mt-2 font-heading text-3xl font-semibold">{success.code}</h1><div className="mt-6 grid grid-cols-2 gap-3 text-left"><SummaryBox label="Wallet deducted" value={formatAed(success.deducted)} /><SummaryBox label="Remaining balance" value={formatAed(success.remaining)} /></div><div className="mt-6 flex flex-col justify-center gap-2 sm:flex-row"><Button asChild><Link href="/agent/bookings">View Booking</Link></Button><Button variant="outline" onClick={reset}>Create Another Booking</Button></div></CardContent></Card></div></AgentPortalShell>;

  return <AgentPortalShell profile={profile} walletBalance={balance}>
    <AgentPageHeader eyebrow="New booking" title="Create a partner booking" description="Complete four concise steps. Final pricing and wallet eligibility remain controlled by the secured booking RPC." actions={<Button variant="outline" onClick={load}><RefreshCw className="size-4" />Refresh balance</Button>} />
    <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_340px]">
      <div>
        ...
        <Card className="mt-4 rounded-2xl border-slate-200"><CardContent className="p-5 sm:p-6">
      ...
      <aside className="xl:sticky xl:top-6 xl:self-start"><Card className="rounded-2xl border-slate-200 shadow-lg"><CardContent className="p-5"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-teal-50 text-teal-700"><WalletCards className="size-5" /></span><div><p className="text-xs font-semibold text-slate-500">Current wallet balance</p><p className="font-heading text-xl font-semibold">{formatAed(balance)}</p></div></div><div className="mt-5 space-y-3 border-t border-slate-200 pt-4"><Row label="B2B unit price" value={formatAed(unit)} /><Row label="Vehicle quantity" value={String(quantity)} /><Row label="Subtotal" value={formatAed(subtotal)} /><Row label="VAT 5%" value={formatAed(vat)} /><Row label="Total wallet debit" value={formatAed(total)} strong /><Row label="Balance after booking" value={formatAed(Math.max(remaining, 0))} strong /></div>{insufficient ? <div className="mt-5 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800"><p className="font-bold">Insufficient wallet balance</p><p className="mt-1">Shortfall: {formatAed(shortfall)}. Call +971 4 611 3114 for top-up assistance.</p></div> : <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-semibold text-emerald-800">Visible balance covers this booking. The secured RPC performs the final check.</div>}</CardContent></Card></aside>
    </div>
  </AgentPortalShell>;
}

function PackageStep({ packages, selectedId, onSelect, quantity, setQuantity }: { packages: PackageRow[]; selectedId: string; onSelect: (id: string) => void; quantity: string; setQuantity: (value: string) => void }) { return <section><h2 className="font-heading text-xl font-semibold">Select package</h2><p className="mt-1 text-sm text-slate-500">Choose the experience and number of vehicles.</p><div className="mt-5 grid gap-3 md:grid-cols-2">{packages.map((item) => <button type="button" key={item.id} onClick={() => onSelect(item.id)} className={`rounded-2xl border p-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-400 ${selectedId === item.id ? 'border-teal-500 bg-teal-50' : 'border-slate-200 hover:border-teal-300'}`}><div className="flex justify-between gap-3"><div><p className="font-semibold">{item.title}</p><p className="mt-1 text-xs text-slate-500">{item.duration_minutes} minutes · Capacity {item.capacity || '-'}</p></div><p className="font-heading font-semibold text-teal-700">{formatAed(item.b2b_price)}</p></div><p className="mt-3 text-xs leading-5 text-slate-500">{item.short_description}</p></button>)}</div><label className="mt-5 grid max-w-xs gap-2 text-sm font-semibold">Vehicle quantity<Input type="number" min="1" max="20" value={quantity} onChange={(e) => setQuantity(e.target.value)} /></label></section>; }
function CustomerStep({ form, update }: { form: FormState; update: (key: keyof FormState, value: string) => void }) { return <section><h2 className="font-heading text-xl font-semibold">Customer details</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Customer name" required value={form.customerName} onChange={(v) => update('customerName', v)} /><Field label="Phone number" required value={form.customerPhone} onChange={(v) => update('customerPhone', v)} /><Field label="Email" type="email" value={form.customerEmail} onChange={(v) => update('customerEmail', v)} /><Field label="Hotel or area" value={form.customerHotelOrArea} onChange={(v) => update('customerHotelOrArea', v)} /><Field label="Guest count" type="number" value={form.guestCount} onChange={(v) => update('guestCount', v)} /><label className="grid gap-2 text-sm font-semibold sm:col-span-2">Customer notes<Textarea value={form.customerNotes} onChange={(e) => update('customerNotes', e.target.value)} /></label></div></section>; }
function ScheduleStep({ form, update }: { form: FormState; update: (key: keyof FormState, value: string) => void }) { return <section><h2 className="font-heading text-xl font-semibold">Schedule</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Preferred date" type="date" min={new Date().toISOString().slice(0, 10)} value={form.preferredDate} onChange={(v) => update('preferredDate', v)} /><label className="grid gap-2 text-sm font-semibold">Preferred time<select value={form.preferredTime} onChange={(e) => update('preferredTime', e.target.value)} className="h-11 rounded-md border border-input bg-white px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-400">{timeSlots.map((time) => <option key={time}>{time}</option>)}</select></label></div></section>; }
function ReviewStep({ form, selected, quantity, total }: { form: FormState; selected: PackageRow | null; quantity: number; total: number }) { return <section><h2 className="font-heading text-xl font-semibold">Review & confirm</h2><div className="mt-5 grid gap-3 sm:grid-cols-2"><SummaryBox label="Package" value={selected?.title || '-'} /><SummaryBox label="Customer" value={form.customerName} /><SummaryBox label="Schedule" value={`${form.preferredDate} · ${form.preferredTime}`} /><SummaryBox label="Vehicles / Guests" value={`${quantity} / ${form.guestCount}`} /><SummaryBox label="Customer phone" value={form.customerPhone} /><SummaryBox label="Total wallet debit" value={formatAed(total)} /></div></section>; }
...
function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) { return <div className={`flex justify-between text-sm ${strong ? 'border-t border-slate-200 pt-3 font-bold' : 'text-slate-600'}`}><span>{label}</span><span>{value}</span></div>; }
function SummaryBox({ label, value }: { label: string; value: string }) { return <div className="rounded-xl bg-slate-50 p-4 text-left"><p className="text-xs font-semibold text-slate-500">{label}</p><p className="mt-1 font-semibold text-slate-900">{value || '-'}</p></div>; }
