...
export function walletTransactionLabel(type: string) {
  const labels: Record<string, string> = {
    wallet_top_up: 'Super Admin Top-up',
  ...
  return (
    <div className="divide-y divide-slate-100">
      {entries.map((entry) => {
        ...
        return (
          <div key={entry.id} className="grid gap-3 px-4 py-4 sm:grid-cols-[auto_1fr_auto] sm:items-center">
            <span className={`flex size-10 items-center justify-center rounded-xl ${credit ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{credit ? <ArrowDownLeft className="size-5" /> : <ArrowUpRight className="size-5" />}</span>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-slate-900">{walletTransactionLabel(entry.transaction_type)}</p>{entry.booking_request_id && bookingCodes[entry.booking_request_id] ? <span className="rounded bg-slate-100 px-2 py-0.5 font-mono text-[11px] text-slate-600">{bookingCodes[entry.booking_request_id]}</span> : null}</div>
            ...
            <div className="text-left sm:text-right"><p className={`font-heading text-lg font-semibold ${credit ? 'text-emerald-700' : 'text-red-700'}`}>{credit ? '+' : '-'}{formatAed(entry.amount_aed)}</p>{!compact ? <p className="mt-1 text-xs text-slate-500">Balance {formatAed(entry.balance_after_aed)}</p> : null}</div>
          </div>
        );
      })}
    </div>
