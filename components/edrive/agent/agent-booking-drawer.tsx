...
export function AgentBookingDrawer({ booking, open, onOpenChange, requestStatus }: {
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent><SheetHeader><SheetTitle>{booking?.booking_code || 'Booking details'}</SheetTitle><SheetDescription>Booking, schedule and wallet pricing details.</SheetDescription></SheetHeader>{booking ? <div className="flex-1 space-y-5 overflow-y-auto p-5">
    ...
    <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><h3 className="font-semibold">Pricing breakdown</h3><Price label="B2B subtotal" value={booking.base_amount_aed || 0} /><Price label="VAT 5%" value={booking.vat_amount || 0} /><Price label="Total wallet debit" value={booking.total_amount || 0} strong /><Price label="Paid" value={booking.amount_received_aed || 0} /><Price label="Pending" value={booking.amount_pending_aed || 0} /></section>
    <section className="rounded-2xl border border-slate-200 p-4"><h3 className="font-semibold">Booking timeline</h3><Timeline label="Booking submitted" value={booking.created_at} /><Timeline label="Booking status" value={booking.status} /><Timeline label="Ride started" value={booking.ride_started_at || 'Not started'} /></section>
...
function Info({ icon: Icon, label, value, detail }: { icon: typeof UserRound; label: string; value: string; detail?: string }) {
  return <div className="rounded-2xl border border-slate-200 p-4"><Icon className="size-4 text-teal-700" /><p className="mt-3 text-xs font-semibold text-slate-500">{label}</p><p className="mt-1 font-semibold text-slate-900">{value}</p>{detail ? <p className="mt-1 text-xs text-slate-500">{detail}</p> : null}</div>;
}
