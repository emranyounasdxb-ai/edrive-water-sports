...
export default function AgentRequestsPage() {
  ...
  return <AgentPortalShell profile={profile} walletBalance={balance}>
    ...
    <div className="mt-5 flex gap-2 overflow-x-auto">{filters.map((item) => <Button key={item} size="sm" variant={filter === item ? 'default' : 'outline'} onClick={() => setFilter(item)}>{item}<span className="ml-1 rounded-full bg-black/10 px-1.5 text-[10px]">{requests.filter((request) => item === 'All' || request.status === item).length}</span></Button>)}</div>
    <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">{!visible.length ? <AgentEmptyState icon={FileClock} title={`No ${filter.toLowerCase()} requests`} description="Requests submitted from an eligible booking will appear here." /> : <div className="divide-y divide-slate-100">{visible.map((request) => { const booking = bookings[request.booking_request_id]; return <article key={request.id} className="grid gap-4 p-5 lg:grid-cols-[1fr_1fr_0.8fr_1.2fr]">
