...
import { Car, ImageIcon, MapPin, Pencil, Plus, Search, Ship, Waves, Wrench, X } from 'lucide-react';
...
export default function Page() {
  ...
  async function setMaintenance(record: FleetRecord) {
    setError('');
    const { error: updateError } = await supabase.from('vehicles').update({ status: 'maintenance', is_available: false }).eq('id', record.id);
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-4 rounded-[1.75rem] border border-white/80 bg-white/85 p-5 shadow-[0_18px_45px_rgba(8,37,50,0.06)] lg:flex-row lg:items-end lg:justify-between">
        ...
        <Button type="button" onClick={() => { setEditing(null); setOpen(true); }}><Plus data-icon aria-hidden="true" />Add Fleet Unit</Button>
      </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            ...
            <label className="grid gap-1.5 text-sm font-semibold text-foreground xl:col-span-2">
              ...
              <span className="relative">
                ...
                <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search code, name, REG no, IMEI..." className="h-10 w-full rounded-xl border border-border bg-white pl-9 pr-3 text-sm text-foreground outline-none focus:border-primary" />
              </span>
            </label>
          </div>
        </CardHeader>
          ...
          <div className="max-h-[70vh] overflow-auto rounded-2xl border border-border/70">
            <Table>
              <TableHeader className="sticky top-0 z-10 bg-white">
                <TableRow>{['Image', 'Code', 'Vehicle', 'Type', 'Location', 'Capacity', 'REG No.', 'Tracker IMEI', 'Status', 'Action'].map((column) => <TableHead key={column}>{column}</TableHead>)}</TableRow>
              </TableHeader>
              <TableBody>
                {filteredItems.length ? filteredItems.map((row) => <TableRow key={row.id}>
                  ...
                  <TableCell className="min-w-[12rem]"><p className="font-semibold text-foreground">{row.name}</p><p className="mt-1 max-w-[15rem] truncate text-xs text-muted-foreground">{row.brand || 'Brand pending'} {row.model || ''}</p></TableCell>
                  ...
                  <TableCell><div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => { setEditing(row); setOpen(true); }}><Pencil className="size-4" aria-hidden="true" />Edit</Button><Button type="button" size="sm" variant="subtle" onClick={() => setMaintenance(row)}>Maintenance</Button></div></TableCell>
                </TableRow>) : <TableRow><TableCell colSpan={10} className="h-28 text-center text-sm text-muted-foreground">{loading ? 'Loading fleet...' : 'No fleet units found for the selected filters.'}</TableCell></TableRow>}
              </TableBody>
            </Table>
          </div>
        </CardContent>
      ...
      {open ? <FleetModal initialValues={editing || undefined} onClose={() => { setOpen(false); setEditing(null); }} onSubmit={saveFleet} /> : null}
    </div>
  );
}
...
function FleetModal({ initialValues, onClose, onSubmit }: { initialValues?: FleetRecord; onClose: () => void; onSubmit: (values: FleetFormValues) => Promise<void> }) {
  ...
  function updateType(value: string) { setValues((current) => ({ ...current, type: value, capacity: value === 'Jet Ski' ? '2' : '4', imageUrl: current.imageUrl || defaultImage(value) })); }
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary-900/35 p-4 backdrop-blur-sm"><div className="flex max-h-[92vh] w-full max-w-[44rem] flex-col overflow-hidden rounded-[1.6rem] border border-white/80 bg-white shadow-[0_28px_80px_rgba(8,37,50,0.28)]"><div className="flex items-start justify-between gap-4 border-b border-border/70 bg-[#F7FAFA] px-5 py-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Fleet Form</p><h2 className="mt-1 font-heading text-xl font-semibold text-foreground">{initialValues ? 'Edit Fleet Unit' : 'Add Fleet Unit'}</h2></div><button type="button" onClick={onClose} className="flex size-9 items-center justify-center rounded-full border border-border bg-white text-muted-foreground transition hover:text-primary"><X className="size-4" aria-hidden="true" /></button></div><form onSubmit={submit} className="flex min-h-0 flex-1 flex-col"><div className="grid gap-3 overflow-y-auto px-5 py-4 sm:grid-cols-2"><label className="grid gap-1.5 text-sm font-semibold text-foreground sm:col-span-2">Fleet Image<div className="flex items-center gap-3 rounded-xl border border-border bg-white p-3"><FleetThumbnail src={values.imageUrl} title={values.name || 'Fleet image'} /><select value={values.imageUrl} onChange={(event) => updateField('imageUrl', event.target.value)} className="h-10 flex-1 rounded-xl border border-border bg-white px-3 text-sm text-foreground outline-none focus:border-primary">{fleetImageOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div></label><FormInput label="Custom Image Path" value={values.imageUrl} placeholder="/images/fleet/jet-car-4-seater.webp" onChange={(value) => updateField('imageUrl', value)} /><FormInput label="Vehicle Code" value={values.code} required placeholder="JC-01 / JS-01" onChange={(value) => updateField('code', value)} /><FormInput label="Vehicle Name" value={values.name} required placeholder="Jet Car 01" onChange={(value) => updateField('name', value)} /><SelectInput label="Vehicle Type" value={values.type} options={Object.keys(typeMap)} required onChange={updateType} /><SelectInput label="Location" value={values.location} options={locations} required onChange={(value) => updateField('location', value)} /><FormInput label="Capacity / Seater" type="number" value={values.capacity} required onChange={(value) => updateField('capacity', value)} /><SelectInput label="Status" value={values.status} options={Object.keys(statusMap)} required onChange={(value) => updateField('status', value)} /><FormInput label="Brand" value={values.brand} onChange={(value) => updateField('brand', value)} /><FormInput label="Model" value={values.model} onChange={(value) => updateField('model', value)} /><FormInput label="Year" type="number" value={values.year} onChange={(value) => updateField('year', value)} /><FormInput label="REG. No." value={values.regNo} onChange={(value) => updateField('regNo', value)} /><FormInput label="DEVICE IMEI" value={values.deviceImei} onChange={(value) => updateField('deviceImei', value)} /><FormInput label="Color" value={values.color} onChange={(value) => updateField('color', value)} /><FormInput label="Date of Installation" type="date" value={values.installationDate} onChange={(value) => updateField('installationDate', value)} /><FormInput label="Expiry Date" type="date" value={values.expiryDate} onChange={(value) => updateField('expiryDate', value)} /><FormInput label="Display Order" type="number" value={values.sortOrder} onChange={(value) => updateField('sortOrder', value)} /><label className="grid gap-1.5 text-sm font-semibold text-foreground sm:col-span-2">Notes<textarea value={values.notes} onChange={(event) => updateField('notes', event.target.value)} className="min-h-20 rounded-xl border border-border bg-white px-3 py-3 text-sm text-foreground outline-none focus:border-primary" /></label></div><div className="flex justify-end gap-3 border-t border-border/70 bg-white px-5 py-4"><Button type="button" variant="outline" onClick={onClose}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? 'Saving...' : 'Save Fleet Unit'}</Button></div></form></div></div>;
}
