...
export function AdminB2BAgentsPolishedPage() {
  ...
  return <section className="w-full px-4 py-4 sm:px-5 lg:px-6">
    <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div className="min-w-0"><p className="truncate text-[10px] font-bold uppercase tracking-[0.14em] text-primary">B2B Partners</p><h1 className="mt-1 truncate font-heading text-2xl font-semibold">Partner Accounts</h1><p className="mt-1 max-w-3xl truncate text-[13px] text-muted-foreground">Manage partner access, company details, rates and account availability.</p></div><div className="flex shrink-0 gap-2"><Button size="sm" variant="outline" onClick={() => load(true)} disabled={refreshing}><RefreshCw className={`size-4 ${refreshing ? 'animate-spin' : ''}`} />Refresh</Button>{canManage ? <Button size="sm" onClick={openAdd}><Plus className="size-4" />Add Agent</Button> : null}</div></div>
