...
export function FinanceReportsPage() {
  ...
  return <section className="space-y-5">
    ...
    <div className="overflow-x-auto rounded-2xl bg-white shadow-sm"><div className="border-b px-4 py-2.5"><h2 className="truncate whitespace-nowrap font-heading text-base font-semibold">{reportType}</h2><p className="text-[11px] text-muted-foreground">{rows.length} filtered records</p></div><table className="w-full min-w-[1100px] text-left text-xs"><thead className="bg-primary-50"><tr>{bookingColumns.map((column) => <th key={column.heading} className="whitespace-nowrap px-3 py-2.5 text-[10px] font-semibold tracking-[0.04em]">{column.heading}</th>)}</tr></thead><tbody>{loading ? <tr><td colSpan={bookingColumns.length} className="p-6 text-center">Loading report...</td></tr> : rows.length ? rows.map((row) => <tr key={String(row.id)} className="h-11 border-t">{bookingColumns.map((column) => <td key={column.heading} className="max-w-56 truncate whitespace-nowrap px-3 py-2">{String(column.value(row) ?? '-')}</td>)}</tr>) : <tr><td colSpan={bookingColumns.length} className="p-6 text-center text-muted-foreground">No records match the applied filters.</td></tr>}</tbody></table></div>
  </section>;
}
