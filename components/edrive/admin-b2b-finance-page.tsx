...
export function AdminB2BFinancePage() {
  ...
  async function load(refresh = false) {
    ...
    try {
      ...
      setAgents(nextAgents); setSelectedId((current) => current || nextAgents[0]?.id || ''); setWallets((walletResult.data || []) as Wallet[]); setLedger((ledgerResult.data || []) as Ledger[]); setRefunds((refundResult.data || []) as Refund[]); setBookings((bookingResult.data || []) as Booking[]);
    } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Unable to load B2B finance.'); }
  ...
  useEffect(() => {
    ...
    getB2BFinanceSummary(selectedId).then((value) => { if (active) setSummary(value); }).catch((summaryError) => { if (active) setError(summaryError instanceof Error ? summaryError.message : 'Unable to load finance summary.'); }).finally(() => { if (active) setSummaryLoading(false); });
    return () => { active = false; };
  ...
  const agentOptions = agents.filter((agent) => `${agent.company_name} ${agent.agent_code}`.toLowerCase().includes(agentSearch.toLowerCase()));
  ...
  async function submitWalletAction() {
    ...
    } catch (actionError) { setError(actionError instanceof Error ? actionError.message : 'Unable to update wallet.'); }
  ...
  async function submitReversal() {
    ...
    catch (reversalError) { setError(reversalError instanceof Error ? reversalError.message : 'Unable to reverse wallet entry.'); }
    finally { setSaving(false); }
  }
  async function submitDecision() {
    ...
    catch (decisionError) { setError(decisionError instanceof Error ? decisionError.message : 'Unable to decide request.'); }
    finally { setSaving(false); }
  }
  ...
  return <section className="w-full px-4 py-5 sm:px-6 lg:px-8">
    ...
    <Card className="mt-5 rounded-2xl border-border/80"><CardContent className="grid gap-3 p-4 lg:grid-cols-[1fr_1.4fr]"><label className="relative"><Search className="absolute left-3 top-3.5 size-4 text-muted-foreground" /><Input className="pl-9" placeholder="Search partner accounts" value={agentSearch} onChange={(event) => setAgentSearch(event.target.value)} /></label><select className={selectClass} aria-label="Selected B2B Agent" value={selectedId} onChange={(event) => setSelectedId(event.target.value)}>{agentOptions.map((agent) => <option key={agent.id} value={agent.id}>{agent.company_name} | {agent.agent_code || 'No code'} | {agent.status} | {formatAed(Number(wallets.find((wallet) => wallet.b2b_agent_id === agent.id)?.balance_aed || 0))}</option>)}</select></CardContent></Card>
    ...
    {canView && selectedAgent ? <>
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><div><p className="font-heading text-xl font-semibold">{selectedAgent.company_name}</p><p className="text-xs text-muted-foreground">{selectedAgent.agent_code} · {selectedAgent.status}</p></div>{canWrite ? <div className="flex flex-wrap gap-2"><Button onClick={() => openAction('fund')}>Add Wallet Funds</Button><Button variant="outline" onClick={() => openAction('credit')}>Manual Credit</Button><Button variant="outline" onClick={() => openAction('debit')}>Manual Debit</Button></div> : <Badge variant="secondary">Finance Read-only</Badge>}</div>
    ...
    <DecisionSheet decision={decision} booking={decision ? bookingMap[decision.request.booking_request_id] : null} agent={selectedAgent} note={note} setNote={setNote} saving={saving} onClose={() => setDecision(null)} onSubmit={submitDecision} />
  </section>;
}
